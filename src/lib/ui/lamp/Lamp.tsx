...
export const Lamp = Component({
    effect: async () => {
        ...
        // Initialize audio
        import("@/assets/click.mp3").then((m) => {
            AUDIO.CLICK = new Audio(m.default);
        });
        ...
        const CORDS = document.querySelectorAll<SVGPathElement>(
            ".toggle-scene__cord"
        );
        const HIT = document.querySelector(".toggle-scene__hit-spot");
        const DUMMY = document.querySelector(".toggle-scene__dummy-cord");
        const DUMMY_CORD = document.querySelector<SVGLineElement>(
        ...
        const CORD_TL = gsap.timeline({
            ...
            onStart: () => {
                STATE.ON = !STATE.ON;
                const lampElement = document.querySelector(".toggle-scene");
                if (lampElement) {
                    gsap.set(lampElement, {
                        "--on": STATE.ON ? 1 : 0
                    });
                }

                // Toggle classes
                document.body.classList.toggle("light", STATE.ON);
                document.body.classList.toggle("dark", !STATE.ON);
